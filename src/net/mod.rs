...
use self::common::prelude::*;
pub use self::worker::{HostSessionConfig, NetSettings, NetWorkerControl};
use bevy::prelude::*;
...
impl Plugin for NetClientPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.configure_set(
            Update,
            NeedsNetSet.run_if(resource_exists::<NetWorkerThread>()),
        )
        ...
        .add_systems(
            ...
            (
                ...
                setup_networkerthread
                    .run_if(resource_added::<NetSettings>())
...
#[derive(Resource)]
pub struct NetControlChannel(pub TxBroadcast<worker::NetWorkerControl>);
...
fn setup_networkerthread(mut commands: Commands, net: Res<NetSettings>) {
    ...
    let networker_settings = net.worker.clone();
...
fn net_gameevent(mut chan: ResMut<GameEventChannel>, mut evw: EventWriter<GameEvent>) {
    while let Ok(ge) = chan.0.try_recv() {
        evw.send(ge);
    }
}

fn net_manager(mut commands: Commands, thread: Option<Res<NetWorkerThread>>) {
...
fn cli_host_connect_last(net: Res<NetSettings>, chan: Res<NetControlChannel>) {
    let config = HostSessionConfig {
        addr: net.last_host_addr.clone(),
        session_id: Some(net.last_host_sessionid),
        server_name: None,
    };
